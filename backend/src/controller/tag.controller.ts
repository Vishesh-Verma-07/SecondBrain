  ...
  try {
    ...
    if (!name) {
      sendResponse(res, 400, {
        status: "error",
        message: "Name is required for tag",
      });
      return;
    }
...
export async function getAllTags(req: Request, res: Response): Promise<void> {
  try {
    ...
    let message = "Failed to fetch tag";
    let statusCode = 500;

    // 2. Safely inspect the unknown error object
    if (error instanceof Error) {
      message = error.message;

      // Check if the error object has a custom statusCode property attached to it
      const errorWithStatus = error as Error & { statusCode?: number };
