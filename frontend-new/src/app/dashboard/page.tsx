...
export default function Dashboard() {
  ...
  const [sort, setSort] = useState<"newest" | "oldest">("newest"),
    ...
    [showCollection, setShowCollection] = useState(false),
      ...
      try {
        ...
        setNotice(
          error instanceof Error
            ? error.message
            : "Could not load your Brain Dock.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const visibleItems = useMemo(() => {
    ...
    return items
      ...
      .sort(
        (a, b) =>
          (semanticResults ? 0 : sort === "newest" ? -1 : 1) *
  ...
  async function search(event: FormEvent) {
    ...
    try {
      ...
      setSemanticResults(true);
      setAnswer(result.answer || "I found relevant items in your Brain Dock.");
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setShowCreate(false);
      setNotice("Saved to your Brain Dock.");
      await loadDashboard();
    } catch (error) {
      setNotice(
  ...
  async function createCollection(event: FormEvent<HTMLFormElement>) {
    ...
    if (name.length < 2)
      return setNotice("Collection names need at least 2 characters.");
    try {
      const created = await brainApi.createCategory(name);
      setCategories((current) => [...current, created]);
      setCollection(created._id);
  ...
  async function remove(id: string, name = "this item", confirmed = false) {
    if (!confirmed) {
      setPendingDelete({ type: "content", id, name });
      return;
    }
    try {
  ...
  async function deleteCollection(
    ...
    if (!confirmed) {
      setPendingDelete({ type: "collection", id, name });
      return;
    }
    try {
      await brainApi.deleteCategory(id);
  ...
  //   if (target.type === "content") await remove(target.id, target.name, true);
  //   else await deleteCollection(target.id, target.name, true);
  // }
        ...
        <div className="collection-list" aria-label="Collections">
          {categories.length ? (
            categories.map((category) => (
              <div className="collection-row" key={category._id}>
                <button
                ...
                <button
                  ...
                  onClick={() => deleteCollection(category._id, category.name)}
      ...
      <section className="dash-main">
        <header className="dash-top">
          <form onSubmit={search}>
            ...
            <input
              ...
              placeholder="Search your Brain Dock..."
              aria-label="Search your Brain Dock"
            />
          </form>
          <Link className="share-button" href="/dashboard/share">
            ↗ Share
          </Link>
          <div
            className="avatar"
        ...
        <div className="dash-content">
          ...
          <div className="content-filters">
            ...
            {(collection !== "all" || selectedTags.length > 0 || query) && (
          ...
          {loading ? (
            ...
            <div className="empty">
              <span>✦</span>
              <h3>Nothing matched these filters</h3>
              <p>Try a different phrase, tag, or collection.</p>
              <button className="button" onClick={() => setShowCreate(true)}>
                ＋ Add an item
              </button>
            </div>
      ...
      {showCreate && (
        <div className="modal-backdrop">
          ...
          >
            ...
            <p className="section-label">ADD TO YOUR BRAIN DOCK</p>
            <h2 id="create-title">
      ...
      {showCollection && (
        <div className="modal-backdrop">
          ...
          >
            ...
            <p className="section-label">NEW COLLECTION</p>
            <h2 id="collection-title">
              Make room for <em>something new.</em>
            </h2>
