...
export default function PublicItemPage() {
  ...
  useEffect(() => {
    brainApi
      ...
      .then((items) => {
        ...
        if (!found) throw new Error("This shared item is unavailable.");
        setItem(found);
      })
      .catch((reason) =>
        setError(
  ...
  return (
    <main className="public-brain public-item">
      <Link className="back-link" href={`/brain/${shareId}`}>
        ← Back to shared Brain Dock
      </Link>
      <p className="section-label">{item.link ? "LINK" : "NOTE"}</p>
      <h1>{item.title}</h1>
      ...
      <div className="public-collections">
        {item.tags?.map((tag) => (
          <span key={tag._id}>{tag.name || tag.title}</span>
        ))}
      </div>
    </main>
  );
